...
Project StarterWare portable yang men-scan bus **I2C1** di SoC AM3352 dan menampilkan hasilnya lewat **UART0** dalam format grid `16Ã—16` ala `i2cdetect -y 1` Linux. Board target-nya **Antminer L3+** â€” sama PCB dan form-factor dengan BeagleBone Black (hanya tanpa PRU), jadi semua pad-mux dan clock-enable di `beaglebone.h` valid.
...
| Pull-up eksternal | **10 kÎ© SCLâ†’3V3 dan SDAâ†’3V3** â€” wajib |
...
```
Folder StarterWare ini dipakai **read-only** â€” tidak ada file StarterWare yang diubah atau dicopy ke project.

---

## âš ï¸� Pull-Up Resistor Wajib

SoC AM3352 memang sudah punya internal pull-up ~50 kÎ© di pad I2C1, tapi **`I2CPinMuxSetup(1)` di StarterWare tidak mengaktifkan bit `PUDEN`** di pin-mux config â€” jadi pull-up internal-nya idle. Untuk I2C di 100 kHz, 50 kÎ© sudah di borderline (rise-time-nya terlalu lambat), dan kalau dipakai external pull-up tidak ada, **slave tidak akan pernah ter-ACK dengan benar**:
...
**Solusi**: solder dua resistor **10 kÎ©** dari:
- P9_17 (SCL) â†’ header VDD_3V3 (P9_3 atau P9_4)
- P9_18 (SDA) â†’ header VDD_3V3 (P9_3 atau P9_4)

Verifikasi cepat dengan multimeter: saat bus idle, SDA dan SCL harus berada di sekitar 3.3 V (bukan floating 1.x V atau 0 V).

> ðŸ’¡ **Device nggak muncul di scan?** Jika pin I2C-nya sudah benar (SCL = P9_17, SDA = P9_18, sudah di-pinmux via `I2CPinMuxSetup(1)`, firmware bisa cetak SCL 100 kHz di scope), **coba dulu pull-up 10 kÎ© ke VDD_3V3 di kedua line** seperti di section [Pull-Up Resistor Wajib](#-pull-up-resistor-wajib). Itu 95% solusinya â€” bukan bug firmware.

---

## Contoh Output UART

Probe loop hanya-baca â€” tidak ada interaksi host. Berikut contoh dengan SSD1306 128Ã—32 OLED di address 0x3C:
...
- kosong (3 spasi) = reserved range (0x00..0x02 general call, 0x78..0x7F 10-bit addrs) â€” tidak di-probe.
...
1. **Project name**: `AM3352_I2C_SCANNER` (atau apa pun â€” nama ini jadi nama ELF output).
2. **Location**: biarkan default (akan dibuat di dalam workspace).
3. **Connection**: pilih **SEGGER J-Link Emulator** (sesuai probe yang dipakai). Asumsinya, sebuah shared global target config **sudah ada** â€” biasanya `AM3352_bb_shared_target_configuration.ccxml` yang tersimpan di user-level config (bukan di dalam folder project), sudah dikonfigurasi untuk probe ini + Cortex-A8 core + device AM3352. Field target configuration cukup dikosongkan atau memilihnya dari dropdown referensi ke config global tersebut. **JANGAN** centang *"Create new target configuration"* â€” kita pakai yang global supaya tidak duplikasi config.
4. **Project type**: `Cortex A.AM3352` (atau `AM335x` generic).
5. Di panel **Project templates and examples**, pilih:
   ```
   â˜‘ Empty Project (with main.c)
   ```
   &nbsp;&nbsp;â†’ otomatis generate `main.c` kosong.
6. **Toolchain**: **TI v20.2.7.LTS** (atau versi LTS CGT ARM yang ter-install â€” bukan GNU ARM).
7. Klik **Finish**.
...
/* UART0 115200 8N1 + I2C1 100 kHz master â€” sama dengan SetupI2C1Master
** di main.c. Probe loop pakai I2C1ProbeAddress(addr) yang polling-only
** (no ISR): bus-busy â†’ ARDY â†’ STOP â†’ STOP_CONDITION, dengan soft-reset
...
```
AM3352_I2C_SCANNER/
â”œâ”€â”€ .ccsproject         â†� toolchain, device, linker cmd, origin path
â”œâ”€â”€ .cproject           â†� full build config (Debug + Release)
â”œâ”€â”€ .project            â†� Eclipse project metadata + linked build artifacts
â”œâ”€â”€ .launches/
â”‚   â””â”€â”€ AM3352_I2C_SCANNER.launch   â†� debug launch config
â”œâ”€â”€ .settings/
...
â”‚   â”œâ”€â”€ AM3352.ccxml            â†� target configuration (opsional, kalau tidak pakai global)
â”‚   â””â”€â”€ readme.txt
â”œâ”€â”€ AM335x.cmd          â†� TI CGT linker command file
â”œâ”€â”€ README.md           â†� you are here
â””â”€â”€ main.c              â†� application source â€” UART bring-up + I2C probe loop
```

Tidak perlu ada `targetConfigs/` sendiri kalau pakai predefined ccxml global â€” CCS akan pakai yang di-set di **Window &rarr; Preferences &rarr; Code Composer Studio &rarr; Debug &rarr; Target Configurations**.
...
1. **Tunggu bus-busy** setelah `I2CMasterStart()` â€” controller owns SDA/SCL.
2. **Tunggu ARDY** (`I2C_INT_ADRR_READY_ACESS`) â€” address byte + ACK/NACK sudah resolved.
3. **Putusan ACK**: ARDY set, `I2C_INT_NO_ACK` clear, `I2C_INT_ARBITRATION_LOST` clear â†’ present=1.
4. **STOP + tunggu STOP_CONDITION** â€” supaya bus idle bersih sebelum probe berikutnya.
5. **Bounded wait + soft-reset** di setiap loop â€” kalau slave hold SDA low (SDA tidak naik karena pull-up lemah, atau slave tidak release), `I2CSoftReset` adalah satu-satunya cara pulih tanpa GPIO bit-bang.
