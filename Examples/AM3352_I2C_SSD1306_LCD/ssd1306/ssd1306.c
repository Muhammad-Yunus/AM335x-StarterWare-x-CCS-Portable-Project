 ...
 * Pixel / font logic kept identical to the STM32 reference; only the
 * bottom-half I2C transport (HAL_I2C_Mem_Write) was replaced by the
 * ssd1306_I2C_WriteCommand() / ssd1306_I2C_WriteData() hooks that the
 * application wires to the StarterWare I2C1 master.
 *
 * DMA mode is disabled in this port (SSD1306_USE_DMA = 0 in
 * ssd1306_defines.h): the StarterWare I2C controller is driven in
 * polling mode, mirroring the I2C1ProbeAddress() flow in the
 * AM3352_I2C_SCANNER baseline.
 */

#include <math.h>
#include "ssd1306.h"

/* StarterWare I2C transport bindings — not part of the portable
 * driver API, but required so the I2C master hooks below can compile
 * against the AM3352 register names and macro definitions. */
...
/* ----- I2C transport (polling, single-shot master TX) -----
 *
 * Single transaction = control byte (0x00=cmd / 0x40=data) + payload.
 * Follows the bounded master-TX pattern used in AM3352_I2C_SCANNER's
 * I2C1ProbeAddress(): drain stale state, set slave addr, set dcount,
 * put control byte, MST_TX, START, wait for ARDY, drain data bytes,
 * STOP, wait for STOP_CONDITION.
 *
 * Limit: keeps payload <= 16 bytes to stay within the controller's
 * FIFO on a single transaction (page-mode buffer updates use 128
 * bytes — handled separately in ssd1306_UpdateScreen() below).
 */
#define I2C_FIFO_BYTES 16
...
static void i2c_master_xmit_blocking(unsigned char *buf, unsigned int len)
{
    volatile unsigned int status = 0;
    unsigned int waited;

    /* Defensive: every transaction gets a clean controller. Otherwise a
     * slave that NACKs or a noisy SDA leaves BB asserted forever. */
    if (I2CMasterBusBusy(SOC_I2C_1_REGS)) {
        I2CMasterStop(SOC_I2C_1_REGS);
        waited = 0;
        while (I2CMasterBusBusy(SOC_I2C_1_REGS) && (waited++ < I2C_TIMEOUT));
        if (I2CMasterBusBusy(SOC_I2C_1_REGS)) {
            i2c_recover();
        }
    }
    ...
    I2CMasterSlaveAddrSet(SOC_I2C_1_REGS, SSD1306_ADDRESS);
    I2CSetDataCount(SOC_I2C_1_REGS, len);
    I2CMasterControl(SOC_I2C_1_REGS, I2C_CFG_MST_TX);

    /* Preload first byte so the controller has something to shift
     * after the auto-generated address byte. */
    I2CMasterDataPut(SOC_I2C_1_REGS, *buf++);
    len--;

    I2CMasterStart(SOC_I2C_1_REGS);

    waited = 0;
    while ((I2CMasterBusBusy(SOC_I2C_1_REGS) == 0) && (waited++ < I2C_TIMEOUT));
    if (!I2CMasterBusBusy(SOC_I2C_1_REGS)) {
        i2c_recover();
        return;
    }

    /* Feed the rest as XRDY lights up. */
    while (len > 0) {
        ...
        if (status & (I2C_INT_NO_ACK | I2C_INT_ARBITRATION_LOST)) {
            break;
        }
    }

    /* Wait for ARDY (all bytes shifted, ACK received). Bail early on NACK
     * or arbitration loss so a stuck slave doesn't wedge the controller. */
    waited = 0;
    do {
        status = I2CMasterIntRawStatus(SOC_I2C_1_REGS);
        if (status & (I2C_INT_NO_ACK | I2C_INT_ARBITRATION_LOST)) break;
        waited++;
    } while (((status & I2C_INT_ADRR_READY_ACESS) == 0) && (waited < I2C_TIMEOUT));
    ...
    do {
        status = I2CMasterIntRawStatus(SOC_I2C_1_REGS);
        waited++;
    } while (((status & I2C_INT_STOP_CONDITION) == 0) && (waited < I2C_TIMEOUT));

    /* Hard-reset if STOP never completed: leaves the controller usable
     * for the next transaction instead of wedging forever. */
...
void ssd1306_I2C_WriteCommand(uint8_t command)
{
    ...
    buf[1] = command;
    i2c_master_xmit_blocking(buf, 2);
}

void ssd1306_I2C_WriteData(uint8_t *data, uint16_t size)
{
    while (size > 0) {
        unsigned char buf[I2C_FIFO_BYTES + 1];
        unsigned int chunk = (size > I2C_FIFO_BYTES) ? I2C_FIFO_BYTES : size;
        unsigned int i;
        buf[0] = 0x40;      /* control byte: Co=0, D/C#=1 -> data */
        for (i = 0; i < chunk; i++) {
            buf[1 + i] = data[i];
        }
        i2c_master_xmit_blocking(buf, chunk + 1);
        data += chunk;
        size -= chunk;
    }
}
